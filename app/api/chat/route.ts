...
export async function POST(req: Request) {
    ...
    const result = await streamText({
        ...
        **面试：**
        ...
        - 询问用户是否进行下一步。注意在面试阶段，你每次只抛出一个问题，然后你会每次的问题都会返回当前面试进度：如，x/5。

        **约束：**
        [IMPORTANT!!!]你的工作流程应该是逐步进行的，不允许一次给出全部的内容。只有让用户确认进行下一步时，才到下一个流程
        [IMPORTANT!!!]在面试过程中，你不直接问出全部问题，一个一个问题
        [IMPORTANT!!!]如果用户发送与面试不相关的回答，请提醒用户继续面试的流程。


        **面试题：**
        根据教育背景、过往经验、技能、沟通与合作能力、学习能力将以下简历生成5个面试问题: ${resumeDatabase.value}`,
